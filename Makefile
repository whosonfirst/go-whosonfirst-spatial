...
debug:
	@make bake
	go run -mod vendor cmd/spatial-server/main.go -enable-www -enable-extras -extras-reader-uri 'whosonfirst:///?reader=fs:///usr/local/data/sfomuseum-data-maps/data&cache=gocache://' -nextzen-apikey $(APIKEY) -mode repo:// /usr/local/data/sfomuseum-data-maps/
